...
import {PointClickApprovalService} from './approvals.mjs';
...
import {startAppWatcher} from './app-watcher.mjs';
...
export async function createService({env = process.env, fetchImpl = globalThis.fetch, approvalService = null, transport = null} = {}) {
  ...
  const approvals = approvalService ?? new PointClickApprovalService({timeoutMs: config.approvals.timeoutMs});
  ...
  await service.init();
  service.appWatcher = startAppWatcher(service, {intervalMs: Number(env.S1R3N_APP_WATCH_MS ?? 5000)});
  return service;
}
